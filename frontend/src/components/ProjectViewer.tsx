		...
		<div className="flex h-screen flex-col bg-canvas">
			<header className="hidden shrink-0 items-center justify-between border-b border-border bg-surface px-4 py-3 lg:flex">
				<div className="flex items-center gap-4">
					<Link
						to="/"
						className="text-lg font-bold text-ink-900 hover:text-ink-900"
					>
						Pennysite
					</Link>
			...
			<header className="flex shrink-0 items-center justify-between border-b border-border bg-surface px-4 py-3 lg:hidden">
				...
				{deployedUrl && (
					<a
						...
						target="_blank"
						rel="noopener noreferrer"
						className="text-sm text-accent-text hover:text-accent"
					>
						View Live
					</a>
				)}
						...
						<div className="space-y-3">
							{conversation.map((msg, i) => {
								if (msg.role === "enhance") {
									return (
										<div
											...
											className={`mr-4 rounded-lg border p-3 ${msg.status === "complete" ? "border-accent/30 bg-accent-light" : msg.status === "error" ? "border-error/30 bg-error/10" : "border-border bg-surface"}`}
										>
											<div className="flex items-center gap-3">
								...
								return (
									<div
										key={`${msg.role}-${i}`}
										className={`text-sm ${msg.role === "user" ? "ml-4 rounded-lg bg-surface-2 p-3 text-ink-900" : "mr-4 rounded-lg bg-surface-2 p-3 text-ink-900"}`}
									>
										{msg.content}
									</div>
								);
						...
						<div className="flex shrink-0 gap-1 overflow-x-auto border-b border-border bg-surface px-4 py-2">
							{pageNames.map((name) => (
								<button
									...
									className={`shrink-0 rounded px-3 py-1 text-sm transition-colors ${currentPage === name ? "bg-surface-2 text-ink-900" : "text-ink-400 hover:bg-surface-2 hover:text-ink-900"}`}
								>
									{name}
								</button>
							))}
			...
			<div className="flex shrink-0 items-center justify-around border-t border-border bg-surface py-2 lg:hidden">
				<button
					...
					className={`flex flex-col items-center gap-1 px-4 py-2 ${mobileTab === "chat" ? "text-ink-900" : "text-ink-400"}`}
				>
					<svg
						xmlns="http://www.w3.org/2000/svg"
						width="20"
				...
				<button
					...
					className={`flex flex-col items-center gap-1 px-4 py-2 ${mobileTab === "preview" ? "text-ink-900" : "text-ink-400"}`}
				>
					<svg
						xmlns="http://www.w3.org/2000/svg"
						width="20"
