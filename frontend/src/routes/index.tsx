...
const jsonLd = {
	...
	url: import.meta.env.VITE_SITE_URL || "https://pennysite.app",
...
export function LandingPage() {
	...
	useEffect(() => {
		document.title = "Pennysite - AI Website Builder";
	}, []);
				...
				<main className="relative flex flex-1 flex-col items-center justify-center px-6 py-12">
					<div className="w-full max-w-3xl text-center">
						<div className="mb-8 inline-flex items-center gap-2 rounded-full border border-[rgba(45,106,79,0.2)] bg-accent-light px-3 py-1 text-xs font-semibold uppercase tracking-wider text-accent-text">
							<span className="text-gold">&#10022;</span> AI Website Builder -
						...
						<div className="mt-4 flex flex-wrap justify-center gap-3">
							<Link
								to="/project/new?prompt=A+warm+elegant+wedding+photography+site+with+a+full-screen+hero+gallery+about+me+section+packages+and+a+contact+form+to+book+a+session"
							...
							<Link
								to="/project/new?prompt=A+bold+motivating+site+for+a+personal+trainer+with+services+like+1-on-1+coaching+and+group+classes+transformation+testimonials+and+a+booking+page"
								className="flex flex-row items-center justify-center rounded-full border border-border bg-white px-4 py-2 text-sm text-ink-600 transition hover:border-accent hover:bg-accent-light hover:text-accent-text"
							>
							...
							<Link
								to="/project/new?prompt=A+clean+modern+landing+page+for+an+indie+podcast+with+episode+highlights+listen-on+links+for+Spotify+and+Apple+a+host+bio+and+an+email+subscribe+form"
								className="flex flex-row items-center justify-center rounded-full border border-border bg-white px-4 py-2 text-sm text-ink-600 transition hover:border-accent hover:bg-accent-light hover:text-accent-text"
							>
